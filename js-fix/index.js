window.addEventListener("load",()=>{
    ...
    console.log(maleBtn);
    let femaleBtn = document.getElementById("btnFemale");
    ...
    let ArrProductShowsHot1 = [];
    for(let i=0;i<8;i++){
        ArrProductShowsHot1.push(productDetail[i]);
    }
    let cntHot = document.querySelector(".cotainer-hot-1");
    console.log(cntHot);
    let stringHot1 = "";
    ArrProductShowsHot1.forEach((item)=>{
       stringHot1+= `<div class="col l-3 m-3 c-6 product-item hot-1">
       <div class="product-sale">
           HOT
       </div>
       <div class="box-img">
           <a href="#" aria-label="CASIO MTP-1370D-7A2VDF – NAM – QUARTZ (PIN) – MẶT SỐ 40MM, KÍNH CỨNG, CHỐNG NƯỚC 5ATM">
               <img  src="${item.img}" alt="" >
           </a>
           <div class="box-text text-center" style="height: 122.287px;">
               <div class="title-wrapper">
                   <p  style="height: 40.7875px;">
                       <a href="" class="product__link">${item.name}</a>
                   </p>
               </div>
               <div class="price-wrapper" style="height: 48.4px; display: flex;
               align-items: center;
    ...
    cntHot.innerHTML = stringHot1;  

    let itemHot1 = document.querySelectorAll(".col.hot-1");
    console.log(itemHot1);

    itemHot1.forEach((item,index)=>{
        item.addEventListener("click",()=>{
            let infoProduct = ArrProductShowsHot1[index].code;
            window.localStorage.setItem("productShow",infoProduct);
            location.href = "cart-detail.html"
        });
    })
    let casioProductArr = [];
    let casioproductShow = [];
    productDetail.forEach((item)=>{
        if(item.brand == 'CASIO'){
            casioProductArr.push(item);
        }
    })
    console.log(casioProductArr);
    
    for(let i=0;i<4;i++){
        casioproductShow.push(casioProductArr[i]);
    }
    console.log(casioproductShow);
    let casiocnt = document.querySelector(".casiocnt"); 
    let stringCasio = "";
    casioproductShow.forEach((item)=>{
        stringCasio+= `
        <div class="col l-3 m-3 c-6 product-item casio-item">
    ...
    stringCasio+=`<button class="col l-4 c-4 m-4 btn-product">
    <a href="" style="text-decoration: none;">
        <span style="color: #545dda; line-height: 40px; font-size:17px; font-weight: bold;">Xem tất cả</span>
    <i class="fa-solid fa-forward" style="font-size: 17px; margin-left: 5px;"></i>
    </a>
</button>`;
    casiocnt.innerHTML=stringCasio;
    let casioItem = document.querySelectorAll(".col.casio-item");
    console.log(casioItem);

    casioItem.forEach((item,index)=>{
        item.addEventListener("click",()=>{
            let infoProduct = casioproductShow[index].code;
            window.localStorage.setItem("productShow",infoProduct);
            location.href = "cart-detail.html"
        });
    })
    ...
    let tissotcnt = document.querySelector(".Tissot-cnt");
    console.log(tissotcnt);
    let tissotString = "";
    let tissotProductArr = [];
    let tissotproductShow = [];
    productDetail.forEach((item)=>{
        if(item.brand == 'TISSOT'){
            tissotProductArr.push(item);
        }
    })
    console.log(tissotProductArr);
    for(let i=0;i<4;i++){
        tissotproductShow.push(tissotProductArr[i]);
    }
    console.log(tissotproductShow);
    tissotproductShow.forEach((item)=>{
        tissotString+=`<div class="col l-3 m-3 c-6 product-item tissot-item">
    ...
    tissotcnt.innerHTML=tissotString;
    let tissotItem = document.querySelectorAll(".tissot-item");
    console.log(tissotItem);
    tissotItem.forEach((item,index)=>{
        item.addEventListener("click",()=>{
        let infoProduct = tissotproductShow[index].code;
        window.localStorage.setItem("productShow",infoProduct);
        location.href = "cart-detail.html"
    });
})
    ...
    let doxatcnt = document.querySelector(".doxa-cnt");
    console.log(tissotcnt);
    let doxaString = "";
    let doxaProductArr = [];
    let doxaproductShow = [];
    productDetail.forEach((item)=>{
        if(item.brand == 'DOXA'){
            doxaProductArr.push(item);
        }
    })
    console.log(doxaProductArr);
    for(let i=0;i<4;i++){
        doxaproductShow.push(doxaProductArr[i]);
    }
    console.log(doxaproductShow);
    doxaproductShow.forEach((item)=>{
        doxaString+=`<div class="col l-3 m-3 c-6 product-item doxa-item">
    ...
    doxatcnt.innerHTML=doxaString;
    let doxaItem = document.querySelectorAll(".doxa-item");
    console.log(tissotItem);
    doxaItem.forEach((item,index)=>{
        item.addEventListener("click",()=>{
        let infoProduct = doxaproductShow[index].code;
        window.localStorage.setItem("productShow",infoProduct);
        location.href = "cart-detail.html"
    });
})
    ...
    let seikocnt = document.querySelector(".seiko-cnt");
    console.log(tissotcnt);
    let seikoString = "";
    let seikoProductArr = [];
    let seikoproductShow = [];
    productDetail.forEach((item)=>{
        if(item.brand == 'SEIKO'){
            seikoProductArr.push(item);
        }
    })
    console.log(seikoProductArr);
    for(let i=0;i<4;i++){
        seikoproductShow.push(seikoProductArr[i]);
    }
    console.log(seikoproductShow);
    seikoproductShow.forEach((item)=>{
        seikoString+=`<div class="col l-3 m-3 c-6 product-item seiko-item">
...
seikocnt.innerHTML = seikoString;
let seikoItem = document.querySelectorAll(".seiko-item");
    console.log(tissotItem);
    seikoItem.forEach((item,index)=>{
        item.addEventListener("click",()=>{
        let infoProduct = seikoproductShow[index].code;
        window.localStorage.setItem("productShow",infoProduct);
        location.href = "cart-detail.html"
    });
})
    ...
    let fossilcnt = document.querySelector(".fossil-cnt");
    console.log(fossilcnt);
    let fossilString = "";
    let fossilProductArr = [];
    let fossilproductShow = [];
    productDetail.forEach((item)=>{
        if(item.brand == 'FOSSIL'){
            fossilProductArr.push(item);
        }
    })
    console.log(fossilProductArr);
    for(let i=0;i<4;i++){
        fossilproductShow.push(fossilProductArr[i]);
    }
    console.log(fossilproductShow);
    fossilproductShow.forEach((item)=>{
        fossilString+=`<div class="col l-3 m-3 c-6 product-item fossil-item">
...
fossilcnt.innerHTML = fossilString;
let fossilItem = document.querySelectorAll(".fossil-item");
    console.log(fossilItem);
    fossilItem.forEach((item,index)=>{
        item.addEventListener("click",()=>{
        let infoProduct = fossilproductShow[index].code;
        window.localStorage.setItem("productShow",infoProduct);
        location.href = "cart-detail.html"
    });
})
